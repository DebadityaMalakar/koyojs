...
// spin up a simulated browser environment
const window = new Window({ url: "https://localhost/" });
const { document } = window;

// patch globals so any library that sniffs window/document behaves as in browser
globalThis.window = window;
globalThis.document = document;
try { Object.defineProperty(globalThis, "navigator", { value: window.navigator, configurable: true }); } catch (_) {}

// import libraries after globals are set
const koyo = await import("../dist/index.mjs");
const { default: _ } = await import("lodash");
const R = await import("remeda");
const radash = await import("radash");
...
const PICK_KEYS = ["key0", "key1", "key2", "key5", "key10"];
const OMIT_KEYS = ["key0", "key1", "key2", "key5", "key10"];
const GROUP_ARR = Array.from({ length: 200 }, (_, i) => ({ type: `t${i % 5}`, val: i }));

// DOM warm-up (proves happy-dom is active)
const root = document.createElement("div");
document.body.appendChild(root);

group("average (1000 numbers) [browser env]", () => {
  bench("koyojs", () => koyo.average(NUMS));
  bench("lodash _.mean", () => _.mean(NUMS));
  bench("remeda R.mean", () => R.mean(NUMS));
});

group("sum (1000 numbers) [browser env]", () => {
...
group("truncate / ellipsify (max 20 chars) [browser env]", () => {
...
group("randomInt (0-1000) [browser env]", () => {
  bench("koyojs", () => koyo.randomInt(0, 1000));
  bench("lodash _.random", () => _.random(0, 1000));
  bench("js baseline", () => Math.floor(window.Math.random() * 1001));
});

group("isEmail [browser env]", () => {
...
// DOM-specific benchmarks
group("DOM createElement + appendChild", () => {
  bench("happy-dom", () => {
    const el = document.createElement("span");
    el.textContent = koyo.capitalize("hello world");
    root.appendChild(el);
    root.removeChild(el);
  });
});

group("DOM querySelector after koyo slugify (class lookup)", () => {
  const slug = koyo.slugify("My Component Title");
  const el = document.createElement("div");
  el.className = slug;
  root.appendChild(el);
  bench("querySelector .slug", () => document.querySelector(`.${slug}`));
  root.removeChild(el);
});

group("DOM textContent via toCamelCase key", () => {
  const el = document.createElement("div");
  root.appendChild(el);
  bench("koyojs toCamelCase → dataset key", () => {
    el.dataset[koyo.toCamelCase("my-data-key")] = "value";
  });
  bench("lodash camelCase → dataset key", () => {
    el.dataset[_.camelCase("my-data-key")] = "value";
  });
  root.removeChild(el);
});

group("chunk (1000 items, size 10) [browser env]", () => {
...
group("groupBy (200 items, 5 groups) [browser env]", () => {
...
group("pick (20-key obj, 5 keys) [browser env]", () => {
  bench("koyojs", () => koyo.pick(OBJ, PICK_KEYS));
  bench("lodash _.pick", () => _.pick(OBJ, PICK_KEYS));
  bench("remeda R.pick", () => R.pick(OBJ, PICK_KEYS));
  bench("radash pick", () => radash.pick(OBJ, PICK_KEYS));
});

group("omit (20-key obj, 5 keys) [browser env]", () => {
  bench("koyojs", () => koyo.omit(OBJ, OMIT_KEYS));
  bench("lodash _.omit", () => _.omit(OBJ, OMIT_KEYS));
  bench("remeda R.omit", () => R.omit(OBJ, OMIT_KEYS));
  bench("radash omit", () => radash.omit(OBJ, OMIT_KEYS));
});

group("cloneDeep (20-key obj) [browser env]", () => {
...
group("createArray / TypedArray push (10 items) [browser env]", () => {
  bench("koyojs createArray (strict number)", () => {
    const arr = koyo.createArray({ strict: true, validate: (v) => typeof v === "number" });
